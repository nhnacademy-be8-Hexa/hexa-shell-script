...
nohup /usr/local/java/java21/bin/java -Dserver.port=$PORT_1 -Dspring.profiles.active=prod -jar ~/api/target/target/api.jar > log_$PORT_1 2>&1 &
...
nohup /usr/local/java/java21/bin/java -Dserver.port=$PORT_2 -Dspring.profiles.active=prod -jar ~/api/target/target/api.jar > log_$PORT_2 2>&1 &
# Health Check 수행
for i in {1..10}; do
    echo "Checking health of the new application on port $DEPLOY_PORT (attempt $i)"
    if curl -s http://127.0.0.1:$PORT_2/actuator/health | grep -q '"status":"UP"'; then
