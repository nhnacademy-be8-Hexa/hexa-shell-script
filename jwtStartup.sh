...
nohup /usr/local/java/java21/bin/java -Dserver.port=$PORT_1 -jar ~/jwt/target/auth-api.jar > log_$PORT_1 2>&1 &
# Health Check 수행
for i in {1..10}; do
    echo "Checking health of the new application on port $PORT_1 (attempt $i)"
    if curl -s http://localhost:$PORT_1/actuator/health | grep -q '"status":"UP"'; then
...
nohup /usr/local/java/java21/bin/java -Dserver.port=$PORT_2 -jar ~/jwt/target/auth-api.jar > log_$PORT_2 2>&1 &
# Health Check 수행
for i in {1..10}; do
